...
const versions = [
  { id: 'airbnb', en: 'ATHENS FLAT', gr: 'ΑΘΕΝΣ FLAT', path: '/airbnb' },
  { id: 'santorini', en: 'SANTORINI', gr: 'ΣΑΝΤΟΡΙΝΗ', path: '/santorini' },
  { id: 'crete', en: 'CRETE', gr: 'ΚΡΗΤΗ', path: '/crete' },
  { id: 'athens', en: 'ATHENS', gr: 'ΡΕΤΙΡΕ', path: '/athens' },
...
export default function DemoSwitcher({ lang = 'en', setLang }) {
  return (
    <motion.div
      ...
      className="fixed top-3 left-0 right-0 z-[999] pointer-events-auto flex justify-center px-3 md:px-[220px]"
    >
      <div className="w-full max-w-[920px] bg-foreground/95 backdrop-blur-xl rounded-full p-1 flex items-center shadow-2xl border border-white/10 overflow-hidden">

        {/* Back to Studio / Home icon */}
        <Link
          to="/"
          title="Back to Studio"
          className="flex-shrink-0 flex items-center gap-2 pl-3 pr-3 lg:pr-4 py-2 border-r border-white/10 text-white/50 hover:text-white transition-colors duration-200"
        >
          <Home size={13} />
        ...
        <div className="flex-1 min-w-0 overflow-x-auto no-scrollbar">
          <div className="flex px-0.5 min-w-max md:min-w-0 md:w-full md:justify-between">
            {versions.map((v) => (
              <NavLink
                key={v.id}
                to={v.path}
                className={({ isActive }) =>
                  `relative px-3 lg:px-4 py-2 text-[10px] lg:text-[11px] font-bold tracking-wide lg:tracking-widest uppercase transition-all duration-300 rounded-full whitespace-nowrap
